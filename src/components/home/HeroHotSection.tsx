                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                    {resources.map((resource) => (
                        <Link key={resource.id} href={`/resources/${resource.id}`} className="block h-full">
                            <Card className="relative overflow-hidden hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300 cursor-pointer h-full flex flex-col border border-slate-100 shadow-sm rounded-2xl bg-white">
                                <div className="relative w-full aspect-[3/2]">
                                    ...
                                    />
                                    <div className="absolute top-2 left-2 px-2 py-0.5 bg-amber-500 text-white text-[10px] font-bold rounded-full shadow-sm">
                                        HOT
                                    </div>
                                </div>
                                <div className="p-3 flex-1 flex flex-col justify-between">
                                    <div>
                                        <div className="text-xs text-blue-600 mb-1 font-medium">{resource.resource_info}</div>
                                        <h3 className="font-bold text-sm line-clamp-2 text-slate-800 group-hover:text-blue-700 transition-colors mb-1">
                                            {resource.title}
                                        </h3>
                                    </div>
